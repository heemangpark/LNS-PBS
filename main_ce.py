...
def CE_collision(info):
    ...
    while itr < max_itr:
        ...
        f, s = np.random.choice(sorted(Counter(collision_agent).items(), key=lambda x: x[1], reverse=True)[0][0], 2,
                                replace=False)
        ...
        itr += 1
        cost = seq_solver(info['grid'], info['agents'], i_a, {'time_limit': 1, 'sub_op': 1.1})[0]
