...
def solver_PBS(instance, agents, tasks, solver_dir, save_dir, exp_name, ret_log=False):
    time_log = dict()
...
def solver_EECBS(instance, agents, tasks, solver_dir, save_dir, exp_name, ret_log=False):
    time_log = dict()
    s_agents = copy.deepcopy(agents)
    todo = copy.deepcopy(tasks)
    seq_paths = [[list(agents[a])] for a in range(len(agents))]
